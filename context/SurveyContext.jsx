...
export function SurveyProvider({ children }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY)
      .then((value) => {
        if (value) setSurveys(JSON.parse(value));
        setLoaded(true);
      })
      .catch(() => setLoaded(true));
  }, []);
  useEffect(() => {
    if (loaded) AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(surveys));
  }, [surveys, loaded]);
  ...
  const value = useMemo(
    () => ({ surveys, draft, setDraft, createSurvey, deleteSurvey }),
    [surveys, draft],
  );
  return (
